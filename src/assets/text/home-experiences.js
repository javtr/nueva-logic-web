export const textEn =
  {
    tit: "Successful Experiences",
    sub: "Discover how our users have successfully transformed their trading with our advanced tools.",
    dec:"Testimonials appearing on this website may not be representative of other customers, and are not a guarantee of future performance or success.",
    tit1: "Marta Melgarejo",
    txt1: "I have made my withdrawal from Uprofit and now I am receiving my withdrawals from APEX, where I have been consistently profitable. This achievement has been reached with the assistance of the indicators. It is important to note that I have maintained my profitability consistently for years. Thank you!",
    tit2: "David Pinzón",
    txt2: "I have managed to obtain more than 10 funded accounts. Thanks to these tools, I have significantly increased my effectiveness percentage and improved my risk-reward ratio. This change has been transformative. To achieve extraordinary results, it is essential to abandon conventional practices and constantly seek exceptional knowledge to adapt to market changes. This experience has transformed not only my operational focus but also my life in general.",
    tit3: "Axel Bravo",
    txt3: "I sincerely appreciate your assistance in the installation and detailed explanation of their operation. It is always important to express gratitude and remember those who provide us with their support.",
    tit4: "James Riaño",
    txt4: "Logic's tools are undoubtedly some of the best available for NinjaTrader today. They load quickly, offer numerous configuration options, and have exceptional customer support that resolves any doubts in a personalized way. They have truly done an impeccable job. Thank you very much!"
  };


export const textEs =
  {
    tit: "Experiencias Exitosas",
    sub: "Descubre cómo nuestros usuarios han transformado su trading con éxito gracias a nuestras herramientas avanzadas.",
    dec:"Es posible que los testimonios que aparecen en este sitio web no son representativos de otros clientes, y no es garantía alguna de futuro rendimiento o éxito",
    tit1: "Marta Melgarejo",
    txt1: "He realizado mi retiro de Uprofit y ahora estoy recibiendo mis retiros de APEX, donde he estado siendo consistente. Este logro lo he alcanzado gracias a la asistencia de los indicadores. Es importante destacar que he mantenido mi rentabilidad de manera consistente durante años. ¡Gracias!",
    tit2: "David Pinzón",
    txt2: "He logrado obtener más de 10 cuentas fondeadas. Gracias a estas herramientas, he podido aumentar significativamente mi porcentaje de efectividad y mejorar mi relación riesgo-beneficio. Este cambio ha sido transformador. Para lograr resultados extraordinarios, es fundamental abandonar las prácticas convencionales y buscar constantemente conocimientos excepcionales para adaptarnos a los cambios del mercado. Esta experiencia ha transformado no solo mi enfoque operativo, sino también mi vida en general.",
    tit3: "Axel Bravo",
    txt3: "Agradezco sinceramente tu ayuda en la instalación y la explicación detallada de su funcionamiento. Siempre es importante expresar gratitud y recordar aquellos que nos brindan su apoyo.",
    tit4: "James Riaño",
    txt4: "Las herramientas de Logic son sin duda alguna unas de las mejores disponibles para NinjaTrader en la actualidad. Se cargan rápidamente, ofrecen numerosas opciones de configuración y cuentan con un soporte al cliente excepcional que resuelve cualquier duda de forma personalizada. Realmente han realizado un trabajo impecable. ¡Muchas gracias!"
  };
